...
public class DaoMessage {
	public static int EnvoyerMessage(Message M ) throws ClassNotFoundException, SQLException  {
		int res = 0;
		connexion.connect();
		res = connexion.Maj("INSERT INTO message(sujet_message , contenu_message , date_envoie, etat_message , id_envoyer , id_recevoir ) VALUES  ('"+M.getSujet_message()+"','"+M.getContenu_message()+"',now(),'-1', "+M.getPersonne_envoyer().getId()+","+M.getPersonne_recevoir().getId()+")");
	...
	public static  ArrayList<Message> ConsulterMessagesParUtilisateur(int ide , int idr ) throws ClassNotFoundException, SQLException {
	 ...
	 public static  ArrayList<Message> ConsulterMessagesSansDuplication(int id) throws ClassNotFoundException, SQLException {
		 ArrayList<Message> messages = new ArrayList<Message>();
			connexion.connect();			
			ResultSet res = connexion.Select("SELECT * FROM `message` WHERE id_message = (SELECT max(id_message) from message where id_recevoir = "+id+")");
			connexion.disconnect();
			try {
			   while (res.next()) 
			   {
