...
/*--------------------- Enregistrert les nouveaux coordonnées ----------------------*/ 
	...
	if ((validateNom()== true) && (validatePrenom()== true) && (validateEmail()== true) && (validateCin()== true) && (validateTel()== true ) && ( validatePassword()== true) )  {
	 	...
	 	$.post("profil",{"nom":nom,"prenom":prenom,"login":login,"cin":cin,"tel":tel,"password":password,"operation":"Modifier_Coordonnee"},
					function(data){
	 		if(data == -1){
	 			swal({
	 	...
	 	var tab=[validateNom(),validatePrenom(),validateEmail(),validateCin(),validateTel(), validatePassword()];
	}
...
function traitementImage(){
    ...
    if(url_img != null ){
		  document.getElementById("buttonimg").style.display = "block";
		}
...
window.onload=function(){
	...
	document.getElementById("photo").addEventListener("click",traitementImage);
    document.getElementById("annulerimg").addEventListener("click",annuler);
