function Reserver(){
	...
	var id=this.parentElement.children[0].value;
	 this.parentElement.parentElement.parentElement.style.display = 'none';
	$.post("enregistrement",{"id_reservation":id,"operation":"mdf"},function(data){
	 });
}
...
function Annuler(){
   ...
   .then((willDelete) => {
     if (willDelete) {
       swal("votre réservation bien annuler!", {
         icon: "success",
       });
       this.parentElement.parentElement.parentElement.style.display = 'none';
       //traitement backend ...
...
window.onload=function(){
	var res =document.getElementsByClassName("reserver");
	var ann =document.getElementsByClassName("annuler");
          
          for (var i =0 ; i<res.length ; i++) {
          	res[i].addEventListener("click", Reserver);
          }
          for (var i =0 ; i<res.length ; i++) {
